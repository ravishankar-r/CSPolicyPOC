...
def evaluate_compliance(event):
    LOG.info("Compliance Evaluation Started")
    client = get_client('iam', event)
    evaluations = []
    response = client.list_users()
    userList = response['Users']
    sts_client = get_client('sts', event)
    res = sts_client.get_caller_identity()["Account"]
    if len(userList) == 0:
        evaluations.append(build_evaluation(res, 'NON_COMPLIANT', event))
    else:
        evaluations.append(build_evaluation('COMPLIANT', 'COMPLIANT', event))
    return evaluations
...
def lambda_handler(event, context):
    ...
    LOG.info("Compliance Evaluation completed")
    return evaluations
...
def build_error_response(internalErrorMessage, internalErrorDetails=None, customerErrorCode=None,
